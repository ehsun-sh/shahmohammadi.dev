...
- **`src/resume/base.typ` owns the typography and both PDFs import it.** The
  measured metrics live there and nowhere else, which is the only reason
  `Portfolio.pdf` can be trusted to look like `resume.pdf`. Changing it changes
  both documents; re-measure the output afterwards, and remember the résumé is
  a reproduction of an approved reference — its content stream hashes to
...
- **`Portfolio.pdf` is private by not existing on the server.** One page per
  ...
  it under `public/`, never add it to the deploy workflow, and if it should
  become public do all three of those at once and gate the link on `site.ts`
  the way `resume.pdf` is. It reads `private/portfolio.json`, built from
  `cv.json` plus the write-ups' frontmatter by
  `scripts/build-portfolio-data.mjs`, because Typst reads JSON and not YAML.
  The markdown body is deliberately absent: a write-up is several pages and
  each project gets one, so the page carries its URL instead.

...
- **The résumé pins `Libertinus Serif` and compiles with
  `--ignore-system-fonts`.** Typst embeds only four faces; any other font
  resolves differently on the CI runner than locally, so a layout approved on
  your machine would ship subtly different. Do not drop that flag or change the
  font to something unembedded. Typst is pinned to 0.15.1 in CI for the same
  reason — this PDF is what someone applies for jobs with.
